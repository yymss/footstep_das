...
ft_time_428[:,1] = ft_time_428[:,0]+600
ft_time_521 = np.array([[41800,42400],[53970,54570],[64150,64750],[72150,72750],[75635,76235],[95635,96235]])
...
for i in range(1,n):
    ...
    z = range(21)
    a = range(1)
    xs_arr = np.linspace(source[i-1,0]-2,source[i-1,0]+1,len(x))
    ys_arr = np.linspace(source[i-1,1]-1,source[i-1,1]+2,len(y))
    theta_arr = np.linspace(0,np.pi,len(z))
    vs_arr = np.linspace(300,310,len(a))
    data3 = data_428[600:1200,10000+int(ft_time2[i-1]):10000+int(ft_time2[i])]
    # 创建参数组合的网格
    ...
    plt.plot(dist*300/300,np.arange(0,len(dist)),'black',linewidth=4)
    plt.xlabel('Time (s)') 
    plt.ylabel('Channel')
    ...
    clim = data_seg.std()*2
    plt.title(str(ft_time[i,0])+','+str(vs_arr[iv_max])+','+str(xs_arr[ixs_max])+','+str(ys_arr[iys_max]))
    plt.imshow(data_seg, aspect='auto', cmap='seismic', vmin = -clim, vmax=clim,alpha=0.7)
    plt.savefig('tmp/428dtsimulation'+str(i)+'.png')
    plt.close()
    np.savetxt('source_428_detailed.txt',source)
    np.savetxt('ft_time_428_detailed.txt',ft_time)
    np.savetxt('v_428_detailed.txt',v)
